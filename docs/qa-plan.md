...
| **P1** — challenge, window open | 2026-09-08 | **PASS.** Day blocks unlock to the current day and no further; `running` state only; Telegram link populated from the dashboard. Found and fixed: the Telegram button showed to non-members, and there was no join CTA at all. |
| **P2** — challenge, past cutoff | 2026-09-08 | **PASS**, forced early by setting `join_cutoff_days` to 1 rather than waiting for 2026-09-13. Both the join button and the `challenge` promo withdraw when joining closes, and both return when the cutoff is set back to 10. Found and fixed: the cutoff retired the promo but not the join button. |
